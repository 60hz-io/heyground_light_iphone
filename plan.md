...
3. ~~오늘 보기 위젯 동작 확인~~ — **2026-08-25 실기기(iPhone 14 / iOS 18.5)에서 검증 완료.**
   결과는 아래 7절에 정리했다
...
## 7. 실기기 검증 결과 (2026-08-25, iPhone 14 / iOS 18.5)

무료 Personal Team 으로 설치해 확인했다.

| 항목 | 결과 |
|---|---|
| 무료 계정으로 App Groups·Keychain Sharing 서명 | **가능** (사전 판단이 틀렸다. 서명된 entitlements 에 둘 다 포함됨) |
| 잠금화면 오른쪽 스와이프 → 오늘 보기에 QR 표시 | **가능. 풀컬러로 그려진다** |
| 잠금 상태에서 위젯 탭으로 갱신 | **불가. 잠금 해제(Face ID)를 요구한다** |
| 잠금화면 Control(iOS 18) 로 앱 열기 | **가능.** Face ID 를 거쳐 QR 화면이 바로 열린다 |

마지막 항목이 이번 설계의 핵심 제약이다. iOS 는 잠금화면에서 인터랙티브 위젯의 동작을 실행하기
전에 인증을 요구한다. 폰을 주운 사람이 동작을 트리거하지 못하게 하는 정책이라 우회할 수 없다.

따라서 실사용 흐름은 이렇게 된다.

```
캐시된 QR 이 유효  → 잠금화면 오늘 보기에서 그대로 스캔 (인증 불필요)
캐시된 QR 이 만료  → 탭 → Face ID → 갱신 → 스캔
확실하게 최신 QR   → 잠금화면 '출입' 버튼 → Face ID → 앱이 열리며 갱신
```

Control 경로는 항상 인증을 거치지만 앱이 열리면서 갱신되므로 **항상 최신 QR** 이 나온다.
만료를 자주 마주친다면 이쪽이 실용적이다.

만료 상태로 마주칠 확률을 줄이는 수단이 지오펜스 prefetch 다. 회사 300m 진입 시 QR 을 새로 받아
두므로, 문까지 걸어오는 몇 분 안에 도착하면 유효한 QR 을 인증 없이 쓸 수 있다.

안드로이드는 알람으로 백그라운드 갱신이 되어 탭이 아예 필요 없지만, iOS 에서는 여기까지가
플랫폼 한계다.

검증 중 발견한 함정: `OpenQRIntent` 를 위젯 확장에만 컴파일하면 Control 버튼이 눌려도 아무 일도
일어나지 않는다. 시스템이 앱을 띄우면서 인텐트를 이어받아야 하므로 앱 타깃에도 포함시켜야 한다
